...
MODEL_PATH = 'DataIntelligenceTeam/BoLInstructions1.0'
lang = 'eng'
# -----------------------------------
...
id2label = {0: 'others',
            ...
            8: 'seal_number',
            9: 'package_quantity',
            10: 'content_description',
            11: 'gross_weight',
            12: 'hs_code'}

header_keys = ['shipper', 'consignee', 'notify', 'port_of_loading', 'port_of_discharge','content_description','hs_code']
details_keys = ['container_id','container_type','package_quantity','gross_weight','seal_number']
# -----------------------------------
...
def structuredResponse(content, n_pag):
    # ---- THIS DEPENDS ON THE MODEL ----
    header_keys = ['shipper', 'consignee', 'notify', 'port_of_loading', 'port_of_discharge','content_description','hs_code']
    det_keys = ['container_id','container_type','package_quantity','gross_weight','seal_number']
    ...
    if str(content) == "test":
        ...
        main_values = ['G20 ENGINEERING S.R.L. LOCALITA CAMPOGRANDE 13 29013 CARPANETO PIACENTINO (PC)',
                       'FACOFRI SARL N° 22 ZONE DACTIVITE — AIN BENIAN ALGER — ALGERIE NIF : 000616097096589',
                       'FACOFRI SARL N° 22 ZONE D ACTIVITE — AIN BENIAN ALGER — ALGERIE NIF : 000616097096589',
                       'MDC',
                       'ALGERI',
                       'KITS CHEMINEES COAXIALES EN PLASTIQUE L = 750 ECHANGEUR DE CHALEUR EN FONTE DEMONTE H.S. CODE 8403909000 FREIGHT PREPAID — BL ORIGINALE 30 GG FREETIME AT DESTINATION',
                       ''
                       ]
        details_values = [['AXIU1378218', '1x40 HC', '23 PACKAGES','4220 KGS','148979']]
    else:
        values = []
        det_values = []
        details = dict()
